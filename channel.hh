...
template<class T>
  ...
  convertToBuffer (const T& data)
  {
    return boost::asio::buffer (reinterpret_cast<const void*> (&data),
				sizeof(T));
  }
...
template<class T>
  ...
  Channel::receive ()
  {
	...
	boost::asio::buffer (&buffer, sizeof(buffer)), ep);

    if (T::SERVICE_TYPE_ID != buffer.header.service_type_identifier)
      {
	throw UnexpectedPackageException (
	    buffer.header.service_type_identifier);
      }
    return buffer;
  }
